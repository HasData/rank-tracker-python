...
def get_datetime_and_rank(query: str, target_domain: str):
    ...
    for page_idx in range(max_pages_search):
        ...
        if not 'organicResults' in data:
            print(f'No more organic results found for --- {query}\nAt page: {page_idx + 1}\n')
            return []
        ...
        for result_idx, result in enumerate(data['organicResults']):
            ...
            if domain_normalized == target_domain:
                ...
                return [datetime_now, search_rank]
...
def main():
    ...
    for query in queries:
        ...
        rank_info = get_datetime_and_rank(query, target_domain)
        if rank_info:
            domain_data[target_domain][query].append(rank_info)
